...
- **Google Drive is enabled** (`GOOGLE_DRIVE_ENABLED = True` in `main.py` and `src/index.tsx`). The OAuth app is published and works for any Google account (checked with an account that was not a test user); only Google's brand verification of the homepage is still pending, since `github.io` isn't accepted as an owned domain. README, PRIVACY and the release now describe Drive, Discord and Steam sharing as shipped features.
- `scripts/package.mjs` now refuses to build a zip when Drive or Discord is enabled but its credentials file is missing, instead of shipping a feature that can't work. The release zip includes the app's OAuth client id/secret (see README's security notes for why that is acceptable).
