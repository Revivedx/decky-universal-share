...
- **Steam sharing**: "Steam (my account)" uploads a screenshot to the user's Steam account with a chosen privacy (Private by default, or Friends only / Unlisted / Public), and "Steam friend (chat)" uploads it as Friends only and sends its link to a friend picked from a searchable list, using Steam's internal chat store (undocumented, so feature-checked and failure-tolerant). New **Share options → Steam** block for the privacy setting.
