...
### Changed
- **README rewritten to be shorter and focused** (about 45% fewer words): the intro says what the plugin does, a "Latest update" section replaces the version line and points here for the full history, and "What it does" and "Why it works this way" are condensed to what matters to users. The obsolete design notes (early custom-capture attempts, the iCloud and Reddit explanations, the modal internals, and a wrong claim that the backend runs as the normal user) and the separate "Google Drive status" section are gone, and the four security sections became one "Security" section answering the practical worry (can someone get my keys?). Download and install comes before the setup guides for Google Drive and Discord.
