...
- There's no filter: a screenshot that happens to show something private will be uploaded like any other, which is the trade-off of not confirming each one. The 30-second delay is a window to delete a screenshot (or switch the toggle off) before it goes out, since the toggles are checked again at upload time.
