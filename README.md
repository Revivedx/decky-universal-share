...
- **Steam sharing runs in the frontend, and friend messages use an undocumented API.** Uploading to a Steam account is something only Steam's own client can do, so it goes through `SteamClient.Screenshots.UploadLocalScreenshot` from the plugin's frontend, not the Python backend. There is no supported way to send an image attachment to a friend, so the plugin uploads the screenshot (Friends only) and sends its community link through Steam's internal chat store. That store is not a documented API and may change with a Steam update; every use is feature-checked, and a failure only produces an error toast. Steam's own chat file upload uses separate web endpoints that need Steam's web session, so it was not used.
...
- The upload privacy is **Private** by default. Choosing **Public** makes every screenshot you upload (including auto-uploads) visible to everyone on your profile, and the settings panel warns about it.
- **Steam friend (chat)** always uploads as *Friends only*, so the link works for your friends and not for the public. If that screenshot was already uploaded as Private, nothing is sent and a toast explains why, because your friend couldn't open it.
...
- Messages are only sent when you pick a friend in the picker. Friends are read from your Steam client's friends list, on the Deck only.
...
- Steam gives plugins no "screenshot taken" event, so new files are found by checking the screenshots folders every few seconds; nothing is read or sent until the delay has passed.
...
- The Discord application's client id/secret live in a gitignored `discord_credentials.json` (see `discord_credentials.example.json`), like the Google ones, and are only bundled in release zips while `DISCORD_ENABLED` is `True`.
